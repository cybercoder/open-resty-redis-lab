...
function _M.new()
    local self = {
        ...
        waf_rules_cache = ngx.shared.waf_rules_cache,
        running = false
    }
...
function _M._run_loop(self)
    ...
    while true do
        ...
        if type(res) == "table" and res[1] == "message" then
            ...
            elseif channel == "invalidate_waf_rules_cache" then
                self.waf_rules_cache:delete(key)
                ngx.log(ngx.INFO, "invalidated waf_rules_cache key: ", key)
            end
        end
